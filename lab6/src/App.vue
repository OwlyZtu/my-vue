<template>
  <div class="max-w-5xl mx-auto p-5">
    ...
    <div class="justify-items-center mb-5">
      <h1 class="text-3xl font-bold mb-5">Планер {{ currentYear }}</h1>
      <h2 class="text-2xl italic">{{ currentMonthName }}</h2>
    </div>
    ...
    <EventForm v-if="showEventForm" :event="selectedEvent" :selected-date="selectedDate" @save="saveEvent"
      ...
      <div class="flex justify-end mb-5 gap-2 bg-green-700 bg-opacity-70 rounded w-1/4 p-3">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="white" class="bi bi-download m-auto" viewBox="0 0 16 16">
          <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5"/>
          <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708z"/>
        </svg>
        <button @click="exportEvents('json')" class="w-24 bg-gray-200 text-black px-4 py-2 rounded hover:bg-gray-400">
          JSON 
        </button>
        <button @click="exportEvents('csv')" class="bg-gray-200 text-black px-4 py-2 rounded hover:bg-gray-400">
          CSV
        </button>
      </div>
    </div>
...
const currentMonthName = computed(() => monthFormatter.format(currentDate.value))
...
const exportEvents = (format) => {
  ...
  try {
    const { content, filename, type } = format === 'json'
      ? {
